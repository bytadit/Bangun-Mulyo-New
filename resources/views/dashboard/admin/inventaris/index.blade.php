...
@section('title') @lang('Data Inventaris - BUMDes Bangun Mulyo') @endsection
@section('css')
...
@section('content')
    @component('components.breadcrumb')
        ...
        @slot('title') Data Inventaris @endslot
    @endcomponent
    <div class="row">
        <div class="col">
            <div class="h-100">
                ...
                <!--end row-->
                @include('dashboard.admin.inventaris.modals.create')
                <div class="row">
                    <div class="col-lg-12">
                        <div class="card">
                                ...
                                <table id="scroll-horizontal" class="table nowrap align-middle table-hover table-bordered" style="width:100%">
                                    ...
                                    @foreach($inventaris as $invent)
                                        <tr>
                                            ...
                                            <td>
                                                {{ \Carbon\Carbon::parse($invent->tgl_pembukuan)->isoFormat('D MMMM Y') }}
                                            </td>
                                            <td>
                                                {{$invent->kode}}
                                            </td>
                                            ...
                                            <td>
                                                @currency($invent->harga_satuan)
                                            </td>
                                            <td>
                                                @currency($invent->jumlah * $invent->harga_satuan)
                                            </td>
                                            <td>
                                                @if($invent->kondisi == 1)
                                                    Baik
                                                @elseif($invent->kondisi == 2)
                                                    Rusak
                                                @endif
                                            </td>
                                            <td>
                                                {{$invent->deskripsi_barang}}
                                            </td>
                                            {{-- <td>
                                                {{$invent->keterangan_tambahan}}
                                            </td> --}}
                                            <td>
                                                <div class="d-flex align-items-center fw-medium">
                                                    <button class="btn btn-sm btn-warning mx-1"  data-bs-toggle="modal" data-bs-target="#editData{{$invent->id}}">
                                                        <i class="ri-pencil-line"></i> <span >@lang('Ubah')</span>
                                                    </button>
                                                    <button class="btn btn-sm btn-danger ml-1" data-bs-toggle="modal" data-bs-target="#deleteData{{$invent->id}}">
                                                        <i class="ri-delete-bin-line"></i> <span >@lang('Hapus')</span>
                                                    </button>
                                                </div>
                                            </td>
                                        </tr>
                                        @include('dashboard.admin.inventaris.modals.edit')
                                        @include('dashboard.admin.inventaris.modals.delete')
                                    @endforeach
                                    </tbody>
                                </table>
                            </div>
